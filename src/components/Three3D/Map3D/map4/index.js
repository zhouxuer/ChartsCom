...
loader.load('./Three3D/map3d/mapDemo2/gdp.json', function (data) {
  ...
  loader.load('./Three3D/map3d/mapDemo2/china.json', function (data2) {
    ...
    data2.features.forEach(function (area) {
      ...
      if (name) {
        ...
        // 颜色插值计算
        const color = color1.clone().lerp(color2.clone(), gdpObj / gdpMax);
        mesh.material.color.copy(color);
        mesh.gdp = gdp; // mesh自定义一个gdp属性，用于标签设置
        mesh.经纬度 = area.properties.center;
...
function choose(event) {
  ...
  // 根据id获取模块
  const mapDox = document.getElementById('map3d4')
  // 容器顶点坐标
